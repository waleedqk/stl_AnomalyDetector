...
from expressionsListener import expressionsListener


class expr_listener(expressionsListener):
    def __init__(self):
        self.stack = {}
        self.signals = []
        self.expr = ""
    ...
    # Exit a parse tree produced by expressionsParser#propFormula.
    def exitPropFormula(self, ctx:expressionsParser.PropFormulaContext):
        self.setExpr(ctx, ctx.getText())

    # Exit a parse tree produced by expressionsParser#signalID.
    def exitSignalID(self, ctx: expressionsParser.SignalIDContext):
        '''
    ...
    # Exit a parse tree produced by expressionsParser#signalBrakedown.
    def exitSignalBrakedown(self, ctx:expressionsParser.SignalBrakedownContext):
        '''
    ...
    # Exit a parse tree produced by expressionsParser#parensSignalFormula.
    def exitParensSignalFormula(self, ctx:expressionsParser.ParensSignalFormulaContext):
        # print("{}".format(self.getExpr(ctx.getChild(1))))
        self.setExpr(ctx, self.getExpr(ctx.getChild(1)))


    # Exit a parse tree produced by expressionsParser#signalFormula.
    def exitSignalFormula(self, ctx: expressionsParser.SignalFormulaContext):
        # print("{}".format(self.getExpr(ctx.getChild(0))))
        self.setExpr(ctx, self.getExpr(ctx.getChild(0)))

    # Exit a parse tree produced by expressionsParser#conjdisjFormula.
    def exitConjdisjFormula(self, ctx:expressionsParser.ConjdisjFormulaContext):
        before = self.getExpr(ctx.getChild(0))
        andor = ctx.andorOp().getText().strip()
        after = self.getExpr(ctx.getChild(2))

        expr = "({} {} {})".format(before, andor, after)
        # print("{}".format(expr))
        self.setExpr(ctx, expr)

    def exitUntilFormula(self, ctx: expressionsParser.UntilFormulaContext):
    ...
    def exitProg(self, ctx:expressionsParser.ProgContext):
        self.expr = self.getExpr(ctx.getChild(0))